...
class Action(nn.Module):
    ...
    def forward(self, x):
        n_batch, c, n_segment, h, w = x.size()  # n_segment <- t, attention: swin-transformer is channel first
        nt = n_batch * n_segment
        ...
        x_excited = (x_p1 + x_p2 + x_p3).view(n_batch, n_segment, -1, h, w).permute(0, 2, 1, 3, 4)
        out = self.net(x_excited)
        return out
    
    
class ConvEmbed(nn.Module):
    def __init__(self, in_c=3, temp_dim=64, embed_dim=96, patch_size=(2,4,4)):
        super().__init__()
        self.conv_exp = nn.Conv3d(in_c, temp_dim, kernel_size=1) # expansion
        self.conv = nn.Conv3d(temp_dim, embed_dim, kernel_size=patch_size, stride=patch_size)
        self.action_conv = Action(net = self.conv, n_segment=8) # 4, 8, 16...
        
    def forward(self, x):
        x = self.conv_exp(x) # demo: input-[16, 3, 8, 224, 224] output-[16, 64, 8, 224, 224]
        x = self.action_conv(x)
        return x
    
    
"""
Video Swin Transformer - ACTION equipped
"""
...
def window_partition(x, window_size: int):
    ...
    # permute: [B, D//Md, Md, H//Mh, Mh, W//Mw, Mw, C] -> [B, D//Md, H//Mh, W//Mh, Md, Mw, Mw, C]
    # view: [[B, D//Md, H//Mh, W//Mh, Md, Mw, Mw, C] -> [B*num_windows, Md*Mh*Mw, C]
...
def window_reverse(windows, window_size, B, D, H, W):
    ...
    Returns:
        x: (B, D, H, W, C)
    """
    # view: [B*num_windows, Md, Mh, Mw, C] -> [B, D//Wd, H//Mh, W//Mw, Md, Mh, Mw, C]
    x = windows.view(B, D // window_size[0], H // window_size[1], W // window_size[2], window_size[0], window_size[1], window_size[2], -1)
    # permute: [B, D//Wd, H//Mh, W//Mw, Md, Mh, Mw, C] -> [B, D//Wd, Md, H//Mh, Mh, W//Mw, Mw, C]
    # view: [B, D//Wd, Md, H//Mh, Mh, W//Mw, Mw, C] -> [B, D, H, W, C]
...
class PatchEmbed3D(nn.Module):
    ...
    """
    def __init__(self, patch_size=(2,4,4), in_c=3, embed_dim=96, norm_layer=None):
        super().__init__()
        ...
        self.proj = ConvEmbed(in_c, temp_dim, embed_dim, patch_size)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

    def forward(self, x):
        _, _, D, H, W = x.shape     # input shape: (batch size, 3, frames, h, w)
        ...
        # 下采样patch_size倍
        x = self.proj(x)    # B C D Mh Mw

        if self.norm is not None:
            D, Mh, Mw = x.size(2), x.size(3), x.size(4)
            # flatten: [B, C, D, Mh, Mw] -> [B, C, DHW]
            # transpose: [B, C, DHW] -> [B, DHW, C]
            x = x.flatten(2).transpose(1, 2)
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.embed_dim, D, Mh, Mw)
        return x, H, W
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None):
        """ Forward function.
        Args:
            x: input features with shape of (num_windows*B, N, C)  N: D*W*H
...
def action_vst(num_classes = 1000, **kwargs):
